...
type Line = {
  startFen: string;
  nodes: PlyNode[];
  preVariations?: Line[]; // variations attached before the first move of this line
};
...
function parseMovetextToTree(moveText: string, startFen?: string): { main: Line; mainlineFlat: PlyNode[] } {
  ...
  function parseLine(idx: number, startFenLocal: string): { line: Line; idx: number } {
    ...
    while (idx < tokens.length) {
      ...
      if (tok.t === TT.RAV_END || tok.t === TT.RESULT) {
        // Line ends here; result/close-paren isn't consumed by caller (except RAV_END)
        if (tok.t === TT.RAV_END) idx++; // consume ')'
        break;
      }

      if (tok.t === TT.RAV_START) {
        // Variation branches from the *current anchor position*.
        // If just played a move, the conventional anchor is position *before* that move.
        const anchorFen = line.nodes.length
          ? line.nodes[line.nodes.length - 1].fenBefore
          : line.startFen;
        ...
        lastWasMove = false; // after a variation, another comment belongs to "before" of the next move
        continue;
      }
      ...
      if (tok.t === TT.MOVE_NUM) {
        idx++;
        lastWasMove = false;
        continue; // informational; we rely on chess turn / fen fullmove to label
      }
      ...
      if (tok.t === TT.SAN) {
        ...
        try {
          ...
          // If SAN token was actually a result or stray token, stop the line gracefully
          // but try to continue if it's just punctuation.
          idx++;
          continue;
        }
  ...
  // Flatten mainline (ignoring variations) for training logic
...
export default function App() {
  ...
  const lastFromTo = useMemo(() => {
    try {
      ...
      for (let i = 1; i <= step; i++) {
        ...
        // Find the move by comparing legal moves — simpler: replay mainline path using SANs is complex,
        // so derive by stepping a copy according to difference; as a heuristic, compute from Square diff.
        // We'll just compute from the actual move using chess history
  ...
  const goToNode = (node: PlyNode) => {
    ...
    // 1) walk ancestors to collect path to the start fen
    const path: PlyNode[] = [];
    ...
    // 2) play ancestor path up to the node
    for (const p of path) {
      try { chess.move(p.sanClean, { sloppy: true }); fens.push(chess.fen()); } catch {}
    }

    // 3) continue along the remainder of the current line (if any)
    if (node.lineRef) {
    ...
    setFenHistory(fens);
    setStep(path.length); // highlight the clicked ply
  ...
  function RenderLine({ line }: { line: Line }) {
    ...
    line.nodes.forEach((node, idx) => {
      ...
      // Variations branching right after this ply's anchor (position before this move)
  ...
  const flow = useMemo(() => {
    ...
    return <RenderLine line={treeMain} />;
