...
const styles = {
  ...
  left: { width: 420 },
  right: { flex: 1, height: "70vh", overflowY: "auto" as const, background: "white", borderWidth: 1, borderStyle: "solid" as const, borderColor: "#e5e7eb", borderRadius: 16, padding: 8 },
  select: { width: "100%", padding: 8, borderRadius: 10, borderWidth: 1, borderStyle: "solid" as const, borderColor: "#d1d5db", background: "white" },
  textarea: { width: "100%", minHeight: 96, padding: 8, borderRadius: 10, borderWidth: 1, borderStyle: "solid" as const, borderColor: "#d1d5db", background: "white" },
  ...
  moveRow: { borderWidth: 1, borderStyle: "solid" as const, borderColor: "#e5e7eb", borderRadius: 12, padding: 8, marginBottom: 8, cursor: "pointer" },
  moveRowActive: { background: "#FEF08A", borderWidth: 1, borderStyle: "solid" as const, borderColor: "#F59E0B" },
  moveNumber: { color: "#6b7280", width: 40, display: "inline-block", textAlign: "right" as const, marginRight: 8 },
...
export default function App() {
  ...
  const moveRefs = useRef<Record<number, HTMLDivElement | null>>({});
  ...
  const animateToStep = (targetStep: number) => {
    ...
    const play = (idx: number) => {
      ...
      setStep(frames[idx]);
      setTimeout(() => play(idx + 1), ANIM_MS + 40);
    };
    play(0);
  };
  ...
  // Autoscroll
  useEffect(() => {
    const el = moveRefs.current[currentPlyIndex];
    if (el && movesPaneRef.current) el.scrollIntoView({ block: "nearest", behavior: "smooth" });
  }, [currentPlyIndex]);
  ...
  }, [fenHistory.length, step, isAnimating]);
  ...
  // Drag & drop: aggiunge allo storico e avanza
  const applyDrop = (sourceSquare: string, targetSquare: string) => {
    const chess = new Chess(fenHistory[step]);
      ...
      <div style={styles.container}>
        <div style={styles.header}>
          <div>
            ...
            <div style={styles.small}>Scacchiera a sinistra, mosse e commenti a destra. Mossa corrente evidenziata in giallo.</div>
          </div>
        ...
        <div style={styles.sectionGrid}>
          <div style={styles.card}>
            <div style={{ marginBottom: 6, fontSize: 12, fontWeight: 600 }}>Partite nel PGN</div>
            <select
              style={styles.select}
            ...
            >
              {games.length ? (
                ...
                <option>(nessuna partita)</option>
              )}
            </select>
          </div>
          <div style={styles.card}>
            <div style={{ marginBottom: 6, fontSize: 12, fontWeight: 600 }}>Oppure incolla qui il PGN</div>
        ...
        <div style={styles.layout}>
          <div style={styles.left}>
            ...
            <div style={{ textAlign: "center", marginTop: 6, ...styles.small }}>
          ...
          <div style={styles.right} ref={movesPaneRef}>
            {plies.length === 0 ? (
              <div style={styles.small}>Carica un PGN e seleziona una partita per vedere mosse e commenti.</div>
            ) : (
              <div>
                {plies.map((ply, i) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ ...styles.moveRow, ...(isActive ? styles.moveRowActive : {}) }}
                      title="Clic per andare a questa mossa"
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 14 }}>
                        <span style={styles.moveNumber}>{label}</span>
                        <span style={{ fontWeight: 700 }}>{ply.san}</span>
                      </div>
                      {(ply.commentBefore || ply.commentAfter) && (
                        <div style={{ marginTop: 4, paddingLeft: 48, fontSize: 12, color: "#374151", whiteSpace: "pre-wrap", fontStyle: "italic" }}>
                          {ply.commentBefore && <div>{ply.commentBefore}</div>}
                          {ply.commentAfter && <div>{ply.commentAfter}</div>}
                        </div>
                      )}
                    </div>
                  );
        ...
        <div style={{ marginTop: 16, ...styles.small }}>
          Suggerimento: usa le frecce ← →, Home/End per navigare. Le varianti ( … ) sono ignorate in questa versione.
        </div>
      </div>
    </div>
  );
